...
def cross_validate(algo, data, measures=['rmse', 'mae'], cv=None,
    ...
    delayed_list = (delayed(fit_and_score)(algo, trainset, testset, measures, data,
...
def primitive(algo, data, n=10):
    ...
    for user in userid:
        ...
        user_list = [x[1] for x in user_list]
        primitive[user] = user_list
    return primitive

def fit_and_score(algo, trainset, testset, measures, data,
    ...
    primitive_list = primitive(algo, data)
    if algo.__class__.__name__ == 'BaselineOnly':
        with open('primitive.json','w') as f:
            json.dump(primitive_list,f)

    return test_measures, train_measures, fit_time, test_time, predictions

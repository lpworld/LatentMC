...
class SVD(AlgoBase):
    ...
    def __init__(self, n_factors=100, n_epochs=1, biased=False, init_mean=0,
                 init_std_dev=.01, lr_all=.005,
        ...
        for current_epoch in range(self.n_epochs):
            ...
            for u, i, r in trainset.all_ratings():
                ...
                for f in range(self.n_factors):
                    puf = pu[u, f]
                    qif = qi[i, f]
                    pu[u, f] += np.average(lr_pu * (err * qif - reg_pu * puf))
                    qi[i, f] += np.average(lr_qi * (err * puf - reg_qi * qif))

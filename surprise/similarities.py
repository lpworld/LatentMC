...
def msd(n_x, yr, min_support):
    ...
    for y, y_ratings in iteritems(yr):
        for xi, ri in y_ratings:
            for xj, rj in y_ratings:
                sq_diff[xi, xj] += spatial.distance.cosine(ri,rj)
                freq[xi, xj] += 1
...
def jaccard(n_x, yr, min_support):
    ...
    for y, y_ratings in iteritems(yr):
        for xi, ri in y_ratings:
            for xj, rj in y_ratings:
                freq[xi, xj] += 1
                keepers = np.where(np.logical_not((np.vstack((ri, rj)) == 0).all(axis=0)))
                prods[xi, xj] += jaccard_similarity_score(np.array(ri)[keepers], np.array(rj)[keepers])
    for xi in range(n_x):
        sim[xi, xi] = 1
        for xj in range(xi + 1, n_x):
            if freq[xi, xj] < min_sprt:
